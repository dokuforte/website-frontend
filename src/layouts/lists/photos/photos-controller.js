...
export default class extends Controller {
  ...
  async renderPhotos() {
    ...
    this.listData.photos.forEach((item, index) => {
      ...
      if (index === 0) {
        // set the meta image
        setPageMeta(null, null, photoRes("large", item.photoId))
      }
    })
  ...
  onPhotoSelected(e) {
    const id = e && e.detail && e.detail.mid ? e.detail.mid : listManager.getSelectedPhotoId()
    const url = `${listManager.getSelectedList().url}/photos/${id}`

    setPageMeta(
      `${this.listData.name} — #${id}`,
      this.listData.description,
      photoRes("large", listManager.getSelectedPhotoId(id).photoId)
    )
  ...
  async removePhoto(e) {
    ...
    if (e && e.currentTarget) {
      ...
      const resp = await listManager.deletePhotoFromList(thumbnail.mid, this.listData.id)
      ...
      trigger("listPhotos:photoRemoved", { id: thumbnail.mid })
    }
  }
