...
const getNextPhotoId = async () => {
  if (photoData.selectedIndex !== -1) {
    ...
    if (nextIndex < photoData.result.items.length) {
      return photoData.result.items[nextIndex].mid
    }
...
const getPrevPhotoId = async () => {
  if (photoData.selectedIndex !== -1) {
    ...
    if (prevIndex >= 0) {
      return photoData.result.items[prevIndex].mid
    }
    ...
    if (result.items && result.items.length) {
      return result.items[result.items.length - 1].mid
    }
