...
export function initUploader (fileInput, iaxios, value, albumid = 0) {
	...
	function listSetFinished (filex) {
		...
		filex.progressText.innerHTML = 
			"<image src='/assets/" + filex.uploadID + "?key=thumb' style='position: relative; float: left; height: 25px; padding-left: 5px; padding-right: 5px; width: 64px;'>"
			+
			"<a target='_blank' href='/assets/" + filex.uploadID + "?key=full'>" + filex.name + "</a>"
		;
